...
public class DialConfig {
	...
	public static void saveDialPhone(Context mContext,String phone){
		...
		mContext.getSharedPreferences(LoginConfig.KC_DEMO, 0).edit().putString(LoginConfig.KC_DEMO+"_DIAL_PHONE", phoneNumber).commit();
	}
	
	public static String[] getDialPhone(Context mContext){
		return mContext.getSharedPreferences(LoginConfig.KC_DEMO, 0).getString(LoginConfig.KC_DEMO+"_DIAL_PHONE", "").split(",");
	}

	
	public static String getCallType(Context mContext){
		return mContext.getSharedPreferences(LoginConfig.KC_DEMO, 0).getString(LoginConfig.KC_DEMO+"_CALL_TYPE", "");
	}
	public static void saveCallType(Context mContext,String type){
		mContext.getSharedPreferences(LoginConfig.KC_DEMO, 0).edit().putString(LoginConfig.KC_DEMO+"_CALL_TYPE", type).commit();
	}
