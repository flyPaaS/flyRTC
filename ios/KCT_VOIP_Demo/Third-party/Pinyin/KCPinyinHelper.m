...
static NSString *FindLetter(int nCode) {
	NSString *strValue = @"";
	switch(nCode) {
		case 6325:
		case 6436:
...
+ (void)convertFromChineseToPinyin:(NSString *)chineseSource
                   firstCharString:(NSMutableString *)firstCharString
                     allCharString:(NSMutableString *)spellCharString
{
    if (!chineseSource || chineseSource.length <= 0) {
        if (firstCharString) {
            [firstCharString setString:@""];
        }
        
        if (spellCharString) {
            [spellCharString setString:@""];
        }
        return;
    }
    ...
    NSData *gb2312_data  = [chineseSource dataUsingEncoding:enc];

    unsigned char ucHigh, ucLow;
    int nCode;
    NSString *allCharArray = @"";
    NSString *firstCharArray = @"";
    ...
    for (int i = 0; i < iLen; i++) {
        ...
        if ((unsigned char)gb2312_string[i] < 0x80 ) {
            NSString *letter = [NSString stringWithFormat:@"%c", gb2312_string[i] > 95 ? gb2312_string[i] - 32 : gb2312_string[i]];
            
            allCharArray = [allCharArray stringByAppendingString:letter];
            firstCharArray = [firstCharArray stringByAppendingString:[letter substringToIndex:1]];
            continue;
        }
        ...
        if (strRes && strRes.length > 0) {
            allCharArray = [allCharArray stringByAppendingString:strRes];
            firstCharArray = [firstCharArray stringByAppendingString:[strRes substringToIndex:1]];
        }
    ...
    // 转换完成
    if (firstCharString){
        [firstCharString setString:firstCharArray];
    }
    if(spellCharString){
        [spellCharString setString:allCharArray];
    }
}

+ (NSString *)quickConvert:(NSString *)hzString
{
    if (nil == hzString || 0 == [hzString length])
    {
        return nil;
    }
    
    char temp = pinyinFirstLetter([hzString characterAtIndex:0]);
...
+ (NSString *)pinyinFromChiniseString:(NSString *)string
{
    ...
    [self.class convertFromChineseToPinyin:string firstCharString:nil allCharString:strValue];
	return [[[NSString alloc] initWithString:strValue] autorelease];
}

@end

// 下面的代码从网上找的转换代码，pinyin.h,pinyin.c
...
char pinyinFirstLetter(unsigned short hanzi)
{
	...
	{
		return firstLetterArray[index] - ('a' - 'A');
	}
