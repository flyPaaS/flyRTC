...
int memcmp(unsigned char* p1, unsigned char* p2, int len)
{
	for (int i=0; i<len; i++)
	{
		if (p1[i] != p2[i])
...
int rsa_check_privkey( rsa_context *ctx )
{
    ...
    mpi_init( &TN); mpi_init( &P1); mpi_init( &Q1); mpi_init( &H); mpi_init( &G);
    ...
    if( mpi_cmp_mpi( &TN, &ctx->N ) == 0 &&
        mpi_cmp_int( &G, 1 ) == 0 )
    {
        mpi_free( &TN); mpi_free( &P1); mpi_free( &Q1); mpi_free( &H); mpi_free( &G);
        return( 0 );
    }
                      ...
                      unsigned char *sig )
{
    ...
    if( len == 34 )
    {
        ...
        if( memcmp( p, (unsigned char*)ASN1_HASH_MDX, 18 ) != 0 )
    ...
    {
        if( memcmp( p, (unsigned char*)ASN1_HASH_SHA1, 15 ) == 0 &&
...
{
    mpi_free( &ctx->RQ); mpi_free( &ctx->RP); mpi_free( &ctx->RN); mpi_free(
              &ctx->QP); mpi_free( &ctx->DQ); mpi_free( &ctx->DP); mpi_free(
              &ctx->Q); mpi_free(  &ctx->P); mpi_free(  &ctx->D); mpi_free(
              &ctx->E); mpi_free(  &ctx->N);
}
