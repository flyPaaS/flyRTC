...
)
{
	...
	if( NULL == metaStruct )
	{
		//printf("NULL == metaStruct \n");
		return -1;
	}
	if( (int)(metaStruct->iSize) != iSize )
	{
		//printf("metaStruct->iSize) != iSize \n");
		return -1;
	}
	for( int i = 0; i < metaStruct->hFieldCount && 0 == ret; i++ ) {
		const SKMetaField_t * field = metaStruct->ptFieldList + i;
        //printf("field name =%s, hType=%d \n", field->pcName, field->hType  );
