...
void MD5::update (uint1 *input, uint4 input_length) {
  ...
  if (finalized){  // so we can't update!
	LogErr ( "MD5::update:  Can't update a finalized digest!" );
    return;
  }
...
void MD5::finalize (){
  ...
  if (finalized){
	LogErr ( "MD5::finalize:  Already finalized this digest!" );
    return;
  }
...
unsigned char *MD5::raw_digest(){
  ...
  if (!finalized){
	LogErr ( "MD5::raw_digest:  Can't get digest if you haven't finalized the digest!" );
    return s;
  }
...
char *MD5::hex_digest(){
  ...
  if (!finalized){
	LogErr ( "MD5::hex_digest:  Can't get digest if you haven't finalized the digest!" );
    return s;
  }
...
std::string MD5::hex_digest2() {
	char * tmpStr = hex_digest();
	std::string rec;
	if (tmpStr != NULL)
	{
		rec = std::string(tmpStr);
		delete tmpStr;
	}
	return rec;
}
