...
const static unsigned short UCS_PROTO_VER_N = htons(ProxyProtocol::PROTO_VER);
const static unsigned short UCS_HEADER_LEN_N = htons(ProxyProtocol::MIN_HEADER_LEN);
...
bool PackageHeader::read(Ucs::ByteBuffer& buffer) {
...
void PackageHeader::write(Ucs::ByteBuffer& buffer) {
    PackageHeader temp;
    temp.packageLength = htonl(packageLength);
    temp.headerLength = UCS_HEADER_LEN_N;
    temp.version = UCS_PROTO_VER_N;
    temp.cmd = htonl(cmd);
    temp.seq = htonl(seq);
