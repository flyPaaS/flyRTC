...
bson * bson_empty2(bson * obj); /* returns pointer to static empty bson object */
void bson_copy2(bson* out, const bson* in); /* puts data in new buffer. NOOP if out==NULL */
...
void bson_print_raw2( const char * bson , int depth );
...
/* more returns true for eoo. best to loop with bson_iterator_next2(&it) */
...
/* calls bson_empty2 on scope if not a bson_codewscope */
...
time_t bson_oid_generated_time2(bson_oid_t* oid); /* Gives the time the OID was created */
...
bson_buffer * bson_append_string2( bson_buffer * b , const char * name , const char * str );
bson_buffer * bson_append_symbol2( bson_buffer * b , const char * name , const char * str );
bson_buffer * bson_append_code2( bson_buffer * b , const char * name , const char * str );
...
bson_buffer * bson_append_bool2( bson_buffer * b , const char * name , const bson_bool_t v );
...
void * bson_malloc2(int size); /* checks return value */
