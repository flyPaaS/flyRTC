...
void add_object_to_bson_buffer2(bson_buffer *bb, id key, id object)
{
    ...
    if ([object isKindOfClass:[NSNumber class]]) {
        ...
        switch (*objCType) {
            case 'd':
            case 'f':
                bson_append_double2(bb, name, [object doubleValue]);
                break;
            case 'l':
            case 'L':
                bson_append_long2(bb, name, [object longValue]);
                break;
            case 'q':
            case 'Q':
                bson_append_long2(bb, name, [object longLongValue]);
                break;
            case 'B':
                bson_append_bool2(bb, name, [object boolValue]);
                break;
            case 'c':
            case 'C':
            case 's':
            ...
            case 'I':
            default:
                bson_append_int2(bb, name, [object intValue]);
                break;
        }
    }
    else if ([object isKindOfClass:[NSDictionary class]]) {
        bson_buffer *sub = bson_append_start_object2(bb, name);
        id keys = [object allKeys];
        for (int i = 0; i < [keys count]; i++) {
            id key = [keys objectAtIndex:i];
            add_object_to_bson_buffer2(sub, key, [object objectForKey:key]);
        }
        bson_append_finish_object2(sub);
    }
    else if ([object isKindOfClass:[NSArray class]]) {
        bson_buffer *arr = bson_append_start_array2(bb, name);
        for (int i = 0; i < [object count]; i++) {
            add_object_to_bson_buffer2(arr,
                [[NSNumber numberWithInt:i] stringValue],
                [object objectAtIndex:i]);
        }
        bson_append_finish_object2(arr);
    }
    else if ([object isKindOfClass:[NSNull class]]) {
        bson_append_null2(bb, name);
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        bson_date_t millis = (bson_date_t) ([object timeIntervalSince1970] * 1000.0);
        bson_append_date2(bb, name, millis);
    }
    else if ([object isKindOfClass:[NSData class]]) {
        bson_append_binary2(bb, name, 0, [object bytes], [object length]);
    }
    else if ([object isKindOfClass:[NuBSONObjectID class]]) {
        bson_append_oid2(bb, name, [((NuBSONObjectID *) object) objectIDPointer]);
    }
        ...
        if ([[object car] isKindOfClass:[NuSymbol class]] && (([object length] % 2) == 0)) {
            ...
            while (cursor && (cursor != [NSNull null])) {
                ...
                add_object_to_bson_buffer2(sub, key, value);
            ...
            int i = 0;
            while (cursor && (cursor != [NSNull null])) {
                add_object_to_bson_buffer2(arr, [[NSNumber numberWithInt:i] stringValue], [cursor car]);
                i++;
                cursor = [cursor cdr];
            }
            bson_append_finish_object2(arr);
        ...
        if ([[object stringValue] isEqualToString:@"t"]) {
            bson_append_bool2(bb, name, YES);
        }
    }
    else if ([object respondsToSelector:@selector(cStringUsingEncoding:)]) {
        bson_append_string2(bb, name, [object cStringUsingEncoding:NSUTF8StringEncoding]);
    }
    else {
...
{
    if (self = [super init]) {
        bson_oid_from_string2(&oid, [s cStringUsingEncoding:NSUTF8StringEncoding]);
    }
    return self;
}
...
- (NSString *) description
{
    ...
    bson_oid_to_string2(&oid, buffer);
...
+ (NSMutableArray *) bsonArrayWithData:(NSData *) data
{
    ...
    bsonBuffer.owned = NO;
    while (bson_size2(&bsonBuffer)) {
        bson bsonValue;
        bson_copy2(&bsonValue, &bsonBuffer);
        bsonBuffer.data += bson_size2(&bsonValue);
        NuBSON *bsonObject = [[[NuBSON alloc] initWithBSON:bsonValue] autorelease];
        [results addObject:bsonObject];
    }
    bson_destroy2(&bsonBuffer);
    return results;
}
...
- (NSData *) dataRepresentation
{
    return [[[NSData alloc]
        ...
        length:bson_size2(&(bsonValue))] autorelease];
}
...
- (NuBSON *) initWithDictionary:(NSDictionary *) dict
{
    bson b;
    bson_buffer bb;
    bson_buffer_init2(& bb );
    id keys = [dict allKeys];
    for (int i = 0; i < [keys count]; i++) {
        id key = [keys objectAtIndex:i];
        add_object_to_bson_buffer2(&bb, key, [dict objectForKey:key]);
    }
    bson_from_buffer2(&b, &bb);
...
- (NuBSON *) initWithList:(id) cell
{
    ...
    while (cursor && (cursor != [NSNull null])) {
        id key = [[cursor car] labelName];
        id value = [[cursor cdr] car];
        add_object_to_bson_buffer2(&bb, key, value);
        cursor = [[cursor cdr] cdr];
    }
    bson_from_buffer2(&b, &bb);
    return [self initWithBSON:b];
...
void dump_bson_iterator(bson_iterator it, const char *indent)
{
    ...
    while(bson_iterator_next2(&it)) {
        fprintf(stderr, "%s  %s: ", indent, bson_iterator_key2(&it));
        char hex_oid[25];

        switch(bson_iterator_type2(&it)) {
            case bson_double:
                fprintf(stderr, "(double) %e\n", bson_iterator_double2(&it));
                break;
            case bson_int:
                fprintf(stderr, "(int) %d\n", bson_iterator_int2(&it));
                break;
            case bson_string:
                fprintf(stderr, "(string) \"%s\"\n", bson_iterator_string2(&it));
                break;
            case bson_oid:
                bson_oid_to_string2(bson_iterator_oid2(&it), hex_oid);
                fprintf(stderr, "(oid) \"%s\"\n", hex_oid);
                break;
            case bson_object:
                fprintf(stderr, "(subobject) {...}\n");
                bson_iterator_subobject2(&it, &subobject);
                bson_iterator_init2(&it2, subobject.data);
...
{
    bson_iterator it;
    bson_iterator_init2(&it, bsonValue.data);
...
id object_for_bson_iterator(bson_iterator it, BOOL expandChildren)
{
    ...
    switch(bson_iterator_type2(&it)) {
        case bson_eoo:
            break;
        case bson_double:
            value = [NSNumber numberWithDouble:bson_iterator_double2(&it)];
            break;
        case bson_string:
            value = [[[NSString alloc]
                initWithCString:bson_iterator_string2(&it) encoding:NSUTF8StringEncoding]
                autorelease];
            break;
        case bson_object:
            if (expandChildren) {
        ...
        case bson_bindata:
            value = [NSData
                dataWithBytes:bson_iterator_bin_data2(&it)
                length:bson_iterator_bin_len2(&it)];
            break;
        case bson_undefined:
            break;
        case bson_oid:
            value = [[[NuBSONObjectID alloc]
                initWithObjectIDPointer:bson_iterator_oid2(&it)]
                autorelease];
            break;
        case bson_bool:
            value = [NSNumber numberWithBool:bson_iterator_bool2(&it)];
            break;
        case bson_date:
            value = [NSDate dateWithTimeIntervalSince1970:(0.001 * bson_iterator_date2(&it))];
            break;
        case bson_null:
            value = [NSNull null];
            break;
        ...
        case bson_codewscope:
            break;
        case bson_int:
            value = [NSNumber numberWithInt:bson_iterator_int2(&it)];
            break;
        case bson_timestamp:
            break;
        case bson_long:
            value = [NSNumber numberWithLong:bson_iterator_long2(&it)];
            break;
        default:
            break;
    }
...
void add_bson_to_object(bson_iterator it, id object, BOOL expandChildren)
{
    while(bson_iterator_next2(&it)) {
        NSString *key = [[[NSString alloc]
            initWithCString:bson_iterator_key2(&it) encoding:NSUTF8StringEncoding]
            autorelease];
...
- (id) objectForKey:(NSString *) key
{
    bson_iterator it;
    bson_iterator_init2(&it, bsonValue.data);
    bson_find2(&it, &bsonValue, [key cStringUsingEncoding:NSUTF8StringEncoding]);
...
{
    add_object_to_bson_buffer2(&bb, key, object);
}
...
- (int) compareDataAtAddress:(void *) aptr withSize:(int) asiz withDataAtAddress:(void *) bptr withSize:(int) bsiz
{
    ...
    bson_iterator it;
    bson_iterator_init2(&it, specification->bsonValue.data);
