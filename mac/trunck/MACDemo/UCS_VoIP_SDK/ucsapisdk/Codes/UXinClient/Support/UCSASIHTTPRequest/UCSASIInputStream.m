...
@implementation UCSASIInputStream

+ (void)initialize
{
	if (self == [UCSASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(UCSASIHTTPRequest *)theRequest
{
	UCSASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(UCSASIHTTPRequest *)theRequest
{
	UCSASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask UCSASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([UCSASIHTTPRequest isBandwidthThrottled]) {
		toRead = [UCSASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[UCSASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
