...
#import <Foundation/Foundation.h>
	...
	#import <CFNetwork/CFNetwork.h>
	 ...
	 // Necessary for background task support
	#endif
#endif
...
@class UCSASIDataDecompressor;

extern NSString *UCSASIHTTPRequestVersion;
...
// The error domain that all errors generated by UCSASIHTTPRequest use
extern NSString* const UCSVoipNetworkRequestErrorDomain;
...
@interface UCSASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the UCSASIHTTPRequestDelegate protocol
	id <UCSASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but UCSASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <UCSASIHTTPRequestDelegate, UCSASIProgressDelegate> queue;
	...
	// Automatically set to true in UCSASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when UCSASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	id <UCSASIProgressDelegate> uploadProgressDelegate;
	...
	id <UCSASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force UCSASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, UCSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, UCSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - UCSASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	UCSASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in UCSASINetworkQueue.h
	...
	// Tells UCSASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let UCSASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// UCSASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// When YES, UCSASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops UCSASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be UCSVoipASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [UCSASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <UCSASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See UCSASICacheDelegate.h for possible values
	ASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See UCSASICacheDelegate.h for possible values
	...
	UCSASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using UCSASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	UCSASIHTTPRequest *PACFileRequest;
...
// Used internally to record bandwidth use, and by UCSASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, UCSASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) UCSASIHTTPRequest *mainRequest;
...
@property (assign) id <UCSASICacheDelegate> downloadCache;
...
#endif
@property (retain) UCSASIDataDecompressor *dataDecompressor;
