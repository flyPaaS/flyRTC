...
//  http://allseeing-i.com/UCSASIHTTPRequest
//
...
#if TARGET_OS_IPHONE
#import "UCSVoipNetworkReachabilityManager.h"
#import "UCSVoipASIAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "UCSASIInputStream.h"
#import "UCSASIDataDecompressor.h"
#import "UCSASIDataCompressor.h"

// Automatically set on build
NSString *UCSASIHTTPRequestVersion = @"v1.8.1-61 2011-09-19";

static NSString *defaultUserAgent = nil;

NSString* const UCSVoipNetworkRequestErrorDomain = @"UCSVoipASIHTTPRequestErrorDomain";

static NSString *UCSVoipASIHTTPRequestRunLoopMode = @"UCSVoipASIHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((UCSASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
// When using UCSASIAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <UCSASICacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [UCSASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(UCSASIHTTPRequest *)theRequest;
...
#if TARGET_OS_IPHONE
+ (void)registerForNetworkUCSReachabilityNotifications;
+ (void)unsubscribeFromNetworkUCSReachabilityNotifications;
// Called when the status of the network changes
+ (void)UCSReachabilityChanged:(NSNotification *)note;
#endif
...
@property (retain, nonatomic) UCSASIHTTPRequest *PACFileRequest;
...
@implementation UCSASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [UCSASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];
		ASIAuthenticationError = [[NSError alloc] initWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		ASIRequestCancelledError = [[NSError alloc] initWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		ASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		ASITooMuchRedirectionError = [[NSError alloc] initWithDomain:UCSVoipNetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[UCSASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UCSASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:ASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UCSASICacheDelegate>)cache andCachePolicy:(ASICachePolicy)policy
{
	UCSASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
    ...
    [delegate release];  // add by baige 2014年09月25日18:10:21
	[super dealloc];
}
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![UCSASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [UCSASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
{
	[[self cancelledLock] lock];
	delegate = [newDelegate retain]; // add by baige 2014年09月25日18:09:16
	[[self cancelledLock] unlock];
}
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [UCSASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:UCSVoipASIHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([UCSASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an UCSASINetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		//If this is a HEAD request generated by an UCSASINetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
		...
		NSError *underlyingError = [NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
			tempUserAgentString = [UCSASIHTTPRequest defaultUserAgentString];
		}
		if (tempUserAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[UCSASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[UCSASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[UCSASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[UCSASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[UCSASIHTTPRequest expirePersistentConnections];
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[UCSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[UCSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[UCSASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
// Used by UCSASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (UCSASIHTTPRequest *)HEADRequest
{
	UCSASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[UCSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[UCSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[UCSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[UCSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[UCSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[UCSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[UCSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[UCSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[UCSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[UCSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[UCSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[UCSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[UCSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[UCSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[UCSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [UCSASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[UCSASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
- (void)failWithError:(NSError *)theError
{
	...
	UCSASIHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an UCSASINetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[UCSASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		UCSASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (BOOL)willRedirect
{
	...
	// Note that UCSASIHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[UCSASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[UCSASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	UCSASIHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an UCSASINetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [UCSASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [UCSASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		ASI_DEBUG_LOG(@"[AUTH] Request %@ received credentials from its delegate or an UCSASIAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		ASI_DEBUG_LOG(@"[AUTH] Request %@ had authentication cancelled by its delegate or an UCSASIAuthenticationDialog",self);
	#endif
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[UCSVoipASIAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
	#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!CFHTTPAuthenticationIsValid(requestAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			if ([self showAuthenticationDialog]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				ASI_DEBUG_LOG(@"[AUTH] Request %@ will ask UCSASIAuthenticationDialog for credentials",self);
				#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[UCSASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[UCSASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[UCSASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[UCSASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
			...
			// Response should already have been inflated, move the temporary file to the destination path
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
	...
	// If request has asked delegate or UCSASIAuthenticationDialog for credentials
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-UCSASIHTTPRequest-Response-Status-Code"] intValue]];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:UCSVoipNetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (UCSASIHTTPRequest does not support file:// urls)
	...
	// Create an UCSASIHTTPRequest to fetch the PAC file
	UCSASIHTTPRequest *PACRequest = [UCSASIHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	UCSASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[UCSASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[UCSASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[UCSASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[UCSASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[UCSASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and UCSASIHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since UCSASIHTTPRequest only works on 10.5+
				OSErr err;
...
- (void)performThrottling
{
	...
	[UCSASIHTTPRequest measureBandwidthUsage];
	if ([UCSASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[UCSASIHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an UCSASINetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[UCSASIHTTPRequest throttleBandwidthForWWANUsingLimit:ASIWWANBandwidthThrottleAmount];
	} else {
		[UCSASIHTTPRequest unsubscribeFromNetworkUCSReachabilityNotifications];
		[UCSASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[UCSASIHTTPRequest registerForNetworkUCSReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[UCSASIHTTPRequest UCSReachabilityChanged:nil];
}

#pragma mark UCSReachability

+ (void)registerForNetworkUCSReachabilityNotifications
{
    [[UCSVoipNetworkReachabilityManager sharedManager]startMonitoring];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(UCSReachabilityChanged:) name:UCSVoipNetworkingReachabilityDidChangeNotification object:nil];
}


+ (void)unsubscribeFromNetworkUCSReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:UCSVoipNetworkingReachabilityDidChangeNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[UCSVoipNetworkReachabilityManager sharedManager] currentNetworkReachabilityStatus] == ReachableViaWWAN);
}

+ (void)UCSReachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [UCSASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <UCSASICacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <UCSASICacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(UCSASIHTTPRequest *)request
{
...
+ (NSDate *)expiryDateForRequest:(UCSASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [UCSASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
