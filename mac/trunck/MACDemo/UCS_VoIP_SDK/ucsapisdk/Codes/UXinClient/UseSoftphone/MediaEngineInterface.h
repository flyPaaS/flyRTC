/***************************************************************************
** ﾰ￦￈ﾨￋ￹ￓ￐:  Copyright (c) 2014-2019
** ￎￄﾼ￾ￃ￻ﾳￆ:  ugo.h
** ￎￄﾼ￾ﾱ￪ￊﾶ: 
** ￄￚ￈￝ￕﾪￒﾪ:  interface define for media engine
** ﾵﾱￇﾰﾰ￦ﾱﾾ:  v1.0
** Author  :  Rambo fu
** ￍ￪ﾳ￉￈ￕￆￚ: 2014ￄ￪9ￔￂ06￈ￕ
** ￐￞ﾸￄﾼￇￂﾼ: 
** ￐￞ﾸￄ￈ￕￆￚ:  2014.9.23
** ﾰ￦ﾱﾾﾺￅ: 
** ￐￞ﾸￄ￈ￋ:    Mao Ningyuan
** ￐￞ﾸￄￄￚ￈￝:  Optimize video interface and struct
***************************************************************************/
...
{
	//ￒￔￏￂￃ﾿ﾸ￶ﾶ￾ￎﾬￊ�ￗ￩ￊￇￓￃￓￚￅ￤ￖￃﾶￔￓﾦﾵￄￖﾵﾣﾬﾵￚￒﾻￎﾬﾴ￺ﾱ￭CPUﾺￋￊ�ﾣﾨANDROID)ﾻ￲ﾻ￺￐ￍﾣﾨIOSﾣﾩﾣﾬﾵￚﾶ￾ￎﾬﾴ￺ﾱ￭￁ￋￊￓￆﾵﾵￄﾷￖﾱ￦ￂￊ
...
class MediaEngineInterface{
 public:
	...
	/* Set video streams */
	virtual	int video_set_stream(const ME_video_stream_info_t* pvideoinfo) = 0;
	/* Create video stream, return 0 if succeed, else -1 */
	virtual int video_create_stream(void) = 0;
	/* Delete video stream, return 0 if succeed, else -1 */
	virtual int video_delete_stream(void) = 0;
	/* Start video, return 0 if succeed */
	virtual int video_start(const ucpaas_uint32 type) = 0;
	/* Stop video, return 0 if succeed */
	virtual int video_stop(const ucpaas_uint32 type) = 0;
	/* Set camera device */
	virtual int video_set_camera(const ME_video_camera_param_t* camera_param) = 0;
	/* Get camera count, return value = camera count */
	virtual int video_get_camera_count(void) = 0;
	/* Get camera state which was used currently */
	virtual int video_get_camera_state(ME_video_camera_param_t* camera_state) = 0;
	/* Get camera info */
	virtual int video_get_camera_info(const ucpaas_uint8 camera_idx, ME_video_camera_info_t* camera_info) = 0;
	/* Get camera Capability */
	virtual int video_get_camera_capability(const ucpaas_uint8 camera_idx, const ucpaas_uint8 cap_idx, ME_video_camera_capability_t* camera_capability) = 0;
	/* Start recording Video, parameter:0-output, 1-input, return 0 if succeed, else -1 */
	virtual int video_start_record(const ME_video_record_param_t* record_param) = 0;
	/* Stop recording Video, parameter:0-output, 1-input, return 0 if succeed, else -1 */
	virtual int video_stop_record(const ucpaas_uint32 stream_select) = 0;

    /* Set Video remote send rotation */
    virtual int video_set_remote_send_rotation(const int angle) = 0;
    
    /* Get Video Send Rotation and State */
    virtual int video_get_sendrotation_state(int& angle, int& state) = 0;
    
	virtual int video_set_remote_dec_resolution(ucpaas_uint32 res) = 0;
	virtual int video_set_remote_camera_resolution(ucpaas_uint32 width, ucpaas_uint32 height) = 0;
	/* set remote dec fps for fps negotiation*/
	virtual int video_set_remote_fps(ucpaas_uint8  fps) = 0;

	virtual int video_set_remote_bitrate(ucpaas_uint32 start_bitrate, ucpaas_uint32 min_bitrate,ucpaas_uint32 max_bitrate) = 0;

	virtual int video_set_send_recive_rotation(ucpaas_uint32 send_rotation, ucpaas_uint32 recive_rotation) = 0;
	
	virtual int video_incoming_frame(const ucpaas_uint8* data, const ucpaas_uint32 len) = 0;

	virtual int screenshot_start(int islocal, ScreenType type) = 0;

	virtual int  video_preset(ucpaas_int8*key, ucpaas_int8*value) = 0;

    /* Set video rotation angle via rtp channel or not */
	virtual int video_set_media_signal_status(int status) = 0;
	...
	virtual int set_test_mode(ucpaas_int32 iMode) = 0;
	virtual int start_RTPDump(const char fileNameUTF8[1024], int direction) = 0;
	virtual int  stop_RTPDump(int direction) = 0;	
	virtual int get_localIP(int& port, char ipAddr[64]) = 0;
	virtual int get_destIP(int& port, char ipAddr[64]) = 0;
