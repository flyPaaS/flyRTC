...
public:
	...
	/* Get camera info */
	virtual int video_get_camera_info(const ucpaas_uint8 camera_idx, ME_video_camera_info_t* camera_info);	
	/* Get camera Capability */
	virtual int video_get_camera_capability(const ucpaas_uint8 camera_idx, const ucpaas_uint8 cap_idx, ME_video_camera_capability_t* camera_capability);
	/* set remote dec resolution for resolution negotiation*/
	virtual int video_set_remote_dec_resolution(ucpaas_uint32 res);
	virtual int video_set_remote_camera_resolution(ucpaas_uint32 width, ucpaas_uint32 height);
	/* set remote dec fps for fps negotiation*/
	virtual int video_set_remote_fps(ucpaas_uint8  fps);

	virtual int video_set_remote_bitrate(ucpaas_uint32 start_bitrate, ucpaas_uint32 min_bitrate,ucpaas_uint32 max_bitrate);

	virtual int video_set_send_recive_rotation(ucpaas_uint32 send_rotation, ucpaas_uint32 recive_rotation);	
