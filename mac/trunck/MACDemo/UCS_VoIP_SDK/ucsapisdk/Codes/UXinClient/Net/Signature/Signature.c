...
void GetRealKey(unsigned char* pKey)
{
	...
	_RC4_set_key(&rc4Key, KEYT_SIZE, KEYT);
	_RC4(&rc4Key, KEY_SIZE, KEY, pKeyEncrypted);*/
	...
	_RC4_set_key(&rc4Key, KEYT_SIZE, (const unsigned char *)KEYT);
	_RC4(&rc4Key, KEY_SIZE / 2, pKeyBinary, pKey);
...
void RC4_s(const unsigned char* pData, const unsigned long nDataLen, unsigned char* pEncrypted)
{
	...
	_RC4_set_key(&rc4Key, RC4_KEY1_SIZE, RC4_KEY1);
	_RC4(&rc4Key, nDataLen, pData, pDataEncrypted1);
	...
	_RC4_set_key(&rc4Key, RC4_KEY2_SIZE, RC4_KEY2);
	_RC4(&rc4Key, nDataLen + RC4_MAIN_KEY_SIZE + strlen(pTemp), pDataEncrypted2, pDataEncryptedFinal);
